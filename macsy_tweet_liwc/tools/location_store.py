...
def worker(macsy_settings, liwc_dict, start_date, end_date):
    ...
    for l in range(1,55):
        ...
        locations[l] = {
            #"fp": np.memmap("{}_{}x{}_tweets.npy".format(l, count, len(labels)), mode="w+", dtype=np.int64, shape=(count, len(labels))),
            "fp": np.lib.format.open_memmap("loc_{}_tweets.npy".format(l), mode="w+", dtype=np.int64, shape=(count, len(labels))),
            "i": 0
        }
